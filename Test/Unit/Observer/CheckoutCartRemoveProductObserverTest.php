...
use \Moosend\WebsiteTracking\Observer\CheckoutCartRemoveProductObserver;
use \Magento\Framework\App\Config\ScopeConfigInterface;
use \Magento\Catalog\Helper\Image;
use \Moosend\WebsiteTracking\Helper\Data;
use \Moosend\TrackerFactory;

class CheckoutCartRemoveProductObserverTestTest extends \PHPUnit\Framework\TestCase
{
	/**
     * @var \Moosend\WebsiteTracking\Observer\CheckoutCartRemoveProductObserver
     */
    protected $observer;
    ...
    public function setUp()
    {
        ...
        $this->observer = $objectManager->getObject(
            CheckoutCartRemoveProductObserver::class,
            array(
    ...
    public function testItShouldTrackRemoveFromCart()
    {
        ...
        $this->productMock = $this->getMockBuilder(\Magento\Catalog\Model\Product::class)
            ->disableOriginalConstructor()
            ->setMethods(array('load', 'getId', 'getImage', 'getProduct', 'getFinalPrice', 'getName', 'getProductUrl', 'getTypeInstance', 'getCategoryIds', 'isInStock', 'getQty'))
            ->getMock();
        ...
        $this->productMock->expects($this->once())->method('getProduct')->will($this->returnValue($this->productMock));
        ...
        $trackerMock->expects($this->once())->method('removeFromOrder')->with($productId, $price, $productUrl, $total, $productName, $productImage);
    ...
    public function getObserverMock($hasSiteId = false)
    {
        ...
        $eventMock = $this->getMockBuilder(\Magento\Framework\Event::class)
            ->disableOriginalConstructor()
            ->setMethods(array('getQuoteItem'))->getMock();

        if ($hasSiteId) {
            $eventObserverMock->expects($this->once())->method('getEvent')->will($this->returnValue($eventMock));
            $eventMock->expects($this->once())->method('getQuoteItem')->willReturn($this->productMock);
        }
